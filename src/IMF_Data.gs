// ---------------------------------------------------------------------------
// IMF_Data.gs — Data fetching and writing
// ---------------------------------------------------------------------------

/**
 * Fetch data from the IMF API for a given dataflow and dimension selection.
 *
 ...
 * @param {string} [options.endPeriod]           End time period (e.g. "2023").
 ...
 */
function imf_fetchData(dataflowId, agency, dimensions, options) {
  options = options || {};
  dimensions = dimensions || {};

  var dsd = imf_getDataStructure(dataflowId, agency);

  var nonTimeDims = dsd.dimensions.filter(function (d) {
    return d.type !== "TimeDimension";
  });
  var key = imf_buildSeriesKey_(dimensions, nonTimeDims);

  var path = "data/dataflow/" + dsd.agency + "/" + dataflowId + "/+/" + key;
  ...
  if (options.startPeriod && options.endPeriod) {
    params["c[TIME_PERIOD]"] =
      "ge:" + options.startPeriod + "+le:" + options.endPeriod;
  } else if (options.startPeriod) {
    params["c[TIME_PERIOD]"] = "ge:" + options.startPeriod;
  } else if (options.endPeriod) {
    params["c[TIME_PERIOD]"] = "le:" + options.endPeriod;
  }
  ...
  var jsonResponse = imfFetch_(path, params);

  return imf_parseSdmxJson_(jsonResponse);
}
 ...
 */
function imf_fetchAndWriteData(dataflowId, agency, dimensions, options, sheetName) {
  sheetName = sheetName || dataflowId;

  var result = imf_fetchData(dataflowId, agency, dimensions, options);
  ...
  return imf_writeDataToSheet_(result.headers, result.rows, sheetName);
}
 ...
 *
 * @param {Object} preset     Preset configuration from IMF_QUICK_SERIES.
 ...
 * @returns {string} Status message.
 */
function imf_quickFetch(preset, overrides) {
  overrides = overrides || {};
  ...
  var countryDim = preset.countryDimension || "COUNTRY";
  ...
  if (overrides.frequency) {
    var freqDim = preset.frequencyDimension || "FREQUENCY";
