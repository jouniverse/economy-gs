// ---------------------------------------------------------------------------
// IMF_Sheet.gs — Write data into Google Sheets
// ---------------------------------------------------------------------------
 ...
 *
 * @param {string[]} headers   Column header names (e.g. ["COUNTRY","TIME_PERIOD","OBS_VALUE"]).
 * @param {any[][]}  rows      2D array of data rows matching the headers.
 * @param {string}   sheetName Name for the new/reused sheet.
 * @returns {string} Status message.
 * @private
 */
function imf_writeDataToSheet_(headers, rows, sheetName) {
  if (!headers || headers.length === 0) {
    throw new Error("No data returned from the IMF API.");
  }
...
/**
 * Write a simple two-column list to a sheet (e.g., dataflows, codes).
 *
 ...
 * @private
 */
function imf_writeListToSheet_(data, sheetName) {
